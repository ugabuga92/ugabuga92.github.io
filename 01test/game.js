const Game = {
    ...
    draw: function() { 
        ...
        for(let y=startY; y<endY; y++) { for(let x=startX; x<endX; x++) { if(y>=0 && y<this.MAP_H && x>=0 && x<this.MAP_W) { 
            ...
            if(['V', 'S', 'C', 'G'].includes(t)) { this.drawTile(ctx, x, y, t, pulse); } 
        } } } 
    ...
    startCombat: function() { let pool = []; if(this.state.zone.includes("Wüste")) pool = [this.monsters.moleRat, this.monsters.raider]; else if(this.state.zone.includes("Supermarkt")) pool = [this.monsters.raider, this.monsters.raider]; else if(this.state.zone.includes("Dschungel")) pool = [this.monsters.mutantRose]; else pool = [this.monsters.moleRat]; if(this.state.lvl >= 5) pool.push(this.monsters.deathclaw); const template = pool[Math.floor(Math.random()*pool.length)]; let enemy = { ...template }; const isLegendary = Math.random() < 0.15; if(isLegendary) { enemy.isLegendary = true; enemy.name = "Legendäre " + enemy.name; enemy.hp *= 2; enemy.maxHp = enemy.hp; enemy.dmg = Math.floor(enemy.dmg*1.5); enemy.loot *= 3; if(Array.isArray(enemy.xp)) enemy.xp = [enemy.xp[0]*3, enemy.xp[1]*3]; } else enemy.maxHp = enemy.hp; this.state.enemy = enemy; this.state.inDialog = true; if(Date.now() < this.state.buffEndTime) UI.log("⚡ S.P.E.C.I.A.L. OVERDRIVE aktiv!", "text-yellow-400"); UI.switchView('combat').then(() => UI.renderCombat()); UI.log(isLegendary ? "LEGENDÄRER GEGNER!" : "Kampf gestartet!", isLegendary ? "text-yellow-400" : "text-red-500"); },
