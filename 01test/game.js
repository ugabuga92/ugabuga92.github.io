const Game = {
    ...
    // [colors, monsters, items hier einfügen wie in v0.0.9c]
    ...
    monsters: { moleRat: { name: "Maulwurfsratte", hp: 25, dmg: 4, xp: [15, 25], loot: 3, minLvl: 1 }, radRoach: { name: "Rad-Kakerlake", hp: 15, dmg: 2, xp: [10, 15], loot: 1, minLvl: 1 }, raider: { name: "Raider", hp: 60, dmg: 12, loot: 20, xp: [50, 70], minLvl: 2 }, ghoul: { name: "Wilder Ghul", hp: 50, dmg: 10, loot: 5, xp: [40, 60], minLvl: 2 }, radScorpion: { name: "Radskorpion", hp: 90, dmg: 18, loot: 15, xp: [80, 100], minLvl: 3 }, mutantRose: { name: "Mutanten-Pflanze", hp: 45, dmg: 15, loot: 15, xp: [45, 60], minLvl: 1 }, superMutant: { name: "Supermutant", hp: 150, dmg: 25, loot: 40, xp: [150, 200], minLvl: 5 }, deathclaw: { name: "Todesklaue", hp: 350, dmg: 60, loot: 150, xp: [500, 700], minLvl: 8 } },
    items: { fists: { name: "Fäuste", slot: 'weapon', baseDmg: 2, bonus: {}, cost: 0, requiredLevel: 0, isRanged: false }, vault_suit: { name: "Vault-Anzug", slot: 'body', bonus: { END: 1 }, cost: 0, requiredLevel: 0 }, knife: { name: "Messer", slot: 'weapon', baseDmg: 6, bonus: { STR: 1 }, cost: 15, requiredLevel: 1, isRanged: false }, pistol: { name: "10mm Pistole", slot: 'weapon', baseDmg: 14, bonus: { AGI: 1 }, cost: 50, requiredLevel: 1, isRanged: true }, leather_armor: { name: "Lederharnisch", slot: 'body', bonus: { END: 2 }, cost: 30, requiredLevel: 1 }, shotgun: { name: "Kampfschrotflinte", slot: 'weapon', baseDmg: 22, bonus: { STR: 1 }, cost: 120, requiredLevel: 3, isRanged: true }, laser_rifle: { name: "Laser-Gewehr", slot: 'weapon', baseDmg: 30, bonus: { PER: 2 }, cost: 300, requiredLevel: 5, isRanged: true }, combat_armor: { name: "Kampf-Rüstung", slot: 'body', bonus: { END: 4 }, cost: 150, requiredLevel: 5 } },
    state: null, worldData: {}, ctx: null, loopId: null, camera: { x: 0, y: 0 }, cacheCanvas: null, cacheCtx: null,

    // MODIFIED INIT: Akzeptiert jetzt saveData
    init: function(saveData) {
        ...
        try {
            if (saveData) {
                ...
                this.state = saveData;
                // Workaround: Items sind in JSON nur Daten, wir müssen Referenzen auf Game.items prüfen falls nötig (hier simple Daten reichen)
                ...
                this.saveGame(); // Direkt initial speichern
            }
    ...
    saveGame: function(manual = false) {
        ...
        // Sende kompletten State an Network
    ...
    gainExp: function(amount) { 
        ...
        while(this.state.xp >= need) { this.state.lvl++; this.state.statPoints++; this.state.xp -= need; need = this.expToNextLevel(this.state.lvl); this.state.maxHp = this.calculateMaxHP(this.getStat('END')); this.state.hp = this.state.maxHp; UI.log(`LEVEL UP! LVL ${this.state.lvl}`, 'text-yellow-400 font-bold'); }
        this.saveGame(); // Autosave bei XP Gain
    },
    ...
    move: function(dx, dy) {
        ...
        if(tile === 'G') {
            ...
            this.saveGame(); // Autosave bei Sektorwechsel
        }
        else if(tile === 'S') UI.enterSupermarket();
        else if(tile === 'H') UI.enterCave();
        else if(tile === 'C') { UI.renderCity(); this.saveGame(); } // Autosave beim Betreten der Stadt
        else if(tile === 'V') { UI.enterVault(); this.saveGame(); } // Autosave beim Betreten der Vault
