const UI = {
    ...
    init: function() {
        ...
        if(this.els.btnLogin) this.els.btnLogin.onclick = () => UI.attemptLogin();
        
        if(this.els.btnToggleRegister) {
            this.els.btnToggleRegister.onclick = () => {
                UI.isRegistering = !UI.isRegistering;
                if(UI.isRegistering) {
                    UI.els.loginTitle.textContent = "NEUEN ACCOUNT ERSTELLEN";
                    UI.els.inputName.style.display = 'block';
                    UI.els.btnLogin.textContent = "REGISTRIEREN";
                    UI.els.btnToggleRegister.textContent = "Zurück zum Login";
                } else {
                    UI.els.loginTitle.textContent = "AUTHENTICATION REQUIRED";
                    UI.els.inputName.style.display = 'none';
                    UI.els.btnLogin.textContent = "LOGIN";
                    UI.els.btnToggleRegister.textContent = "Noch kein Account? Hier registrieren";
                }
            };
        }

        [this.els.inputEmail, this.els.inputPass, this.els.inputName].forEach(el => {
            if(el) el.addEventListener("keydown", (e) => { if (e.key === "Enter") { e.preventDefault(); UI.attemptLogin(); } });
        });

        if (this.els.btnCharSelectAction) this.els.btnCharSelectAction.onclick = () => UI.triggerCharSlot();
        if (this.els.btnCharDeleteAction) this.els.btnCharDeleteAction.onclick = () => UI.triggerDeleteSlot();
        if (this.els.btnCreateCharConfirm) this.els.btnCreateCharConfirm.onclick = () => { const name = UI.els.inputNewCharName.value.trim().toUpperCase(); if(name.length < 3) { alert("Name zu kurz!"); return; } UI.els.newCharOverlay.classList.add('hidden'); UI.startGame(null, UI.selectedSlot, name); };
        if (this.els.btnDeleteCancel) this.els.btnDeleteCancel.onclick = () => UI.closeDeleteOverlay();
        
        if (this.els.deleteInput) {
            this.els.deleteInput.addEventListener('input', () => {
                const target = UI.els.deleteTargetName.textContent;
                const input = UI.els.deleteInput.value.toUpperCase();
                UI.els.btnDeleteConfirm.disabled = (target !== input);
                if(target === input) {
                    UI.els.btnDeleteConfirm.classList.remove('border-red-500', 'text-red-500');
                    UI.els.btnDeleteConfirm.classList.add('border-green-500', 'text-green-500', 'animate-pulse');
                } else {
                    UI.els.btnDeleteConfirm.classList.add('border-red-500', 'text-red-500');
                    UI.els.btnDeleteConfirm.classList.remove('border-green-500', 'text-green-500', 'animate-pulse');
                }
            });
        }
        ...
        if(this.els.btnDeleteConfirm) {
            this.els.btnDeleteConfirm.onclick = async () => {
                ...
                UI.closeDeleteOverlay();
                UI.attemptLogin(); 
            };
        }

        if(this.els.btnSave) this.els.btnSave.onclick = () => UI.handleSaveClick();
        if(this.els.btnMenuSave) this.els.btnMenuSave.onclick = () => UI.handleSaveClick();
        if(this.els.btnLogout) this.els.btnLogout.onclick = () => UI.logout('MANUELL AUSGELOGGT');
        if(this.els.btnReset) this.els.btnReset.onclick = () => UI.handleReset();
        if(this.els.btnConfirmReset) this.els.btnConfirmReset.onclick = () => UI.confirmReset();
        if(this.els.btnCancelReset) this.els.btnCancelReset.onclick = () => UI.cancelReset();
        if(this.els.btnMenu) this.els.btnMenu.onclick = (e) => { e.stopPropagation(); UI.toggleMenu(); };

        document.addEventListener('click', (e) => {
            if(UI.els.navMenu && !UI.els.navMenu.classList.contains('hidden')) {
                if (!UI.els.navMenu.contains(e.target) && e.target !== UI.els.btnMenu) {
                    UI.els.navMenu.classList.add('hidden');
                    UI.els.navMenu.style.display = 'none';
            ...
            if (Game.state && Game.state.view !== 'map' && Game.state.view !== 'combat' && Game.state.view !== 'city' && !Game.state.view.includes('shop') && !Game.state.view.includes('crafting') && !Game.state.view.includes('clinic') && !Game.state.view.includes('vault')) {
                 ...
                 if (!viewContainer.contains(e.target)) UI.switchView('map');
            }
        });
        ...
        if(this.els.log.parentElement) {
            this.els.log.parentElement.addEventListener('click', () => {
                if (Game.state && Game.state.view !== 'map' && Game.state.view !== 'combat' && !Game.state.view.includes('city')) {
                     if (Game.state.view !== 'combat') UI.switchView('map');
                }
            });
        }

        if(this.els.playerCount) this.els.playerCount.onclick = () => UI.togglePlayerList();
        if(this.els.btnInv) this.els.btnInv.onclick = () => UI.toggleView('inventory');
        if(this.els.btnWiki) this.els.btnWiki.onclick = () => UI.toggleView('wiki');
        if(this.els.btnMap) this.els.btnMap.onclick = () => UI.toggleView('worldmap');
        if(this.els.btnChar) this.els.btnChar.onclick = () => UI.toggleView('char');
        if(this.els.btnQuests) this.els.btnQuests.onclick = () => UI.toggleView('quests');
        if(this.els.btnSpawnRandom) this.els.btnSpawnRandom.onclick = () => UI.selectSpawn(null);

        if(this.els.touchArea) {
            this.els.touchArea.addEventListener('touchstart', (e) => UI.handleTouchStart(e), {passive: false});
            this.els.touchArea.addEventListener('touchmove', (e) => UI.handleTouchMove(e), {passive: false});
            this.els.touchArea.addEventListener('touchend', (e) => UI.handleTouchEnd(e));
            this.els.touchArea.addEventListener('touchcancel', (e) => UI.handleTouchEnd(e));
        }

        window.addEventListener('keydown', (e) => {
            if (UI.deleteMode) return; 
            if (UI.charSelectMode) {
                if (e.key === 'ArrowUp') UI.navigateCharSlot(-1);
                if (e.key === 'ArrowDown') UI.navigateCharSlot(1);
                if (e.key === 'Enter') UI.triggerCharSlot();
                if (e.key === 'Delete' || e.key === 'Backspace') UI.triggerDeleteSlot();
                return;
            }
            if (!Game.state || Game.state.isGameOver) {
                if(UI.els.gameOver && !UI.els.gameOver.classList.contains('hidden') && (e.key === 'Enter' || e.key === ' ')) location.reload();
                return;
            }
            if(e.key === 'Escape') {
                if (Game.state.inDialog) { return; }
                else if(UI.els.playerList && UI.els.playerList.style.display === 'flex') UI.togglePlayerList();
                else if(UI.els.navMenu && !UI.els.navMenu.classList.contains('hidden')) UI.toggleMenu();
                else if(Game.state.view !== 'map') UI.switchView('map'); 
                else UI.toggleMenu(); 
                return;
            }
            if (Game.state.inDialog) {
                if (['ArrowUp', 'ArrowDown', 'ArrowLeft', 'ArrowRight', 'w', 'a', 's', 'd'].includes(e.key)) {
                    UI.navigateFocus(e.key === 'ArrowRight' || e.key === 'd' || e.key === 'ArrowDown' || e.key === 's' ? 1 : -1);
                } else if (e.key === 'Enter' || e.key === ' ') { UI.triggerFocus(); }
                return;
            }
            ...
            if (Game.state.view !== 'map' || isMenuOpen) {
                ...
                if (['ArrowUp', 'w'].includes(e.key)) UI.navigateFocus(isGrid ? -4 : -1); 
                if (['ArrowDown', 's'].includes(e.key)) UI.navigateFocus(isGrid ? 4 : 1);
                if (['ArrowLeft', 'a'].includes(e.key)) UI.navigateFocus(-1);
                if (['ArrowRight', 'd'].includes(e.key)) UI.navigateFocus(1);
                if (e.key === 'Enter' || e.key === ' ') UI.triggerFocus();
                return;
            }
    ...
    renderCharacterSelection: function(saves) {
        ...
        for (let i = 0; i < 5; i++) {
            ...
            slot.onclick = () => UI.selectSlot(i); this.els.charSlotsList.appendChild(slot);
        }
        UI.selectSlot(0);
    },
    selectSlot: function(index) {
        ...
        if (this.els.btnCharSelectAction) {
            if (save) { this.els.btnCharSelectAction.textContent = "SPIEL LADEN"; this.els.btnCharSelectAction.className = "action-button w-full border-green-500 text-green-500 font-bold py-3 mb-2"; if(this.els.btnCharDeleteAction) { this.els.btnCharDeleteAction.classList.remove('hidden'); this.els.btnCharDeleteAction.style.display = 'flex'; } } 
            else { this.els.btnCharSelectAction.textContent = "CHARAKTER ERSTELLEN"; this.els.btnCharSelectAction.className = "action-button w-full border-yellow-400 text-yellow-400 font-bold py-3 mb-2"; if(this.els.btnCharDeleteAction) { this.els.btnCharDeleteAction.classList.add('hidden'); this.els.btnCharDeleteAction.style.display = 'none'; } }
        }
    },
    navigateCharSlot: function(delta) { let newIndex = this.selectedSlot + delta; if(newIndex < 0) newIndex = 4; if(newIndex > 4) newIndex = 0; UI.selectSlot(newIndex); },
    triggerCharSlot: function() { if(this.selectedSlot === -1) return; const save = this.currentSaves[this.selectedSlot]; if(save) { UI.startGame(save, this.selectedSlot); } else { this.els.newCharOverlay.classList.remove('hidden'); this.els.inputNewCharName.value = ""; this.els.inputNewCharName.focus(); } },
    triggerDeleteSlot: function() { if(this.selectedSlot === -1) return; const save = this.currentSaves[this.selectedSlot]; if(!save) return; this.deleteMode = true; this.els.deleteOverlay.style.display = 'flex'; this.els.deleteTargetName.textContent = save.playerName; this.els.deleteInput.value = ""; this.els.btnDeleteConfirm.disabled = true; this.els.btnDeleteConfirm.classList.add('border-red-500', 'text-red-500'); this.els.btnDeleteConfirm.classList.remove('border-green-500', 'text-green-500', 'animate-pulse'); this.els.deleteInput.focus(); },
    ...
    showJoystick: function(x, y) { if(!this.els.joyBase) UI.restoreOverlay(); if(this.els.joyBase) { this.els.joyBase.style.left = (x - 50) + 'px'; this.els.joyBase.style.top = (y - 50) + 'px'; this.els.joyBase.style.display = 'block'; } if(this.els.joyStick) { this.els.joyStick.style.left = (x - 25) + 'px'; this.els.joyStick.style.top = (y - 25) + 'px'; this.els.joyStick.style.display = 'block'; } },
    ...
    togglePlayerList: function() { if(this.els.playerList.style.display === 'flex') { this.els.playerList.style.display = 'none'; } else { UI.updatePlayerList(); this.els.playerList.style.display = 'flex'; UI.focusIndex = -1; UI.refreshFocusables(); } },
    ...
    switchView: async function(name) { UI.stopJoystick(); UI.focusIndex = -1; if(this.els.navMenu) { this.els.navMenu.classList.add('hidden'); this.els.navMenu.style.display = 'none'; } if(this.els.playerList) this.els.playerList.style.display = 'none'; const verDisplay = document.getElementById('version-display'); const ver = verDisplay ? verDisplay.textContent.trim() : Date.now(); if (name === 'map') { this.els.view.innerHTML = ` <div id="map-view" class="w-full h-full flex justify-center items-center bg-black relative"> <canvas id="game-canvas" class="w-full h-full object-contain" style="image-rendering: pixelated;"></canvas> <button onclick="UI.switchView('worldmap')" class="absolute top-4 right-4 bg-black/80 border-2 border-green-500 text-green-500 p-2 rounded-full hover:bg-green-900 hover:text-white transition-all z-20 shadow-[0_0_15px_#39ff14] animate-pulse cursor-pointer" title="Weltkarte öffnen"> <span class="text-2xl">🌍</span> </button> </div>`; Game.state.view = name; Game.initCanvas(); UI.restoreOverlay(); UI.toggleControls(true); UI.updateButtonStates(name); UI.update(); return; } const path = `views/${name}.html?v=${ver}`; try { const res = await fetch(path); if (!res.ok) throw new Error(`View '${name}' not found`); const html = await res.text(); this.els.view.innerHTML = html; Game.state.view = name; UI.restoreOverlay(); if (name === 'combat') { UI.toggleControls(false); if(typeof Combat !== 'undefined' && typeof Combat.render === 'function') Combat.render(); else UI.renderCombat(); } else { UI.toggleControls(false); } if (name === 'char') UI.renderChar(); if (name === 'inventory') UI.renderInventory(); if (name === 'wiki') UI.renderWiki(); if (name === 'worldmap') UI.renderWorldMap(); if (name === 'city') UI.renderCity(); if (name === 'combat') UI.renderCombat(); if (name === 'quests') UI.renderQuests(); if (name === 'crafting') UI.renderCrafting(); UI.updateButtonStates(name); UI.update(); setTimeout(() => UI.refreshFocusables(), 100); } catch (e) { this.error(`Ladefehler: ${e.message}`); } },
    ...
    showPermadeathWarning: function() {
        if(!this.els.dialog) UI.restoreOverlay();
