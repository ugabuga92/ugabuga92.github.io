Object.assign(UI, {
    ...
    renderSpawnList: function(players) { 
        ...
        if(count === 0) { this.els.spawnList.innerHTML = '<div class="text-center italic opacity-50 p-2">Keine Signale. Starte Solo.</div>'; } 
    },
    
    togglePlayerList: function() { 
        if(this.els.playerList.style.display === 'flex') { this.els.playerList.style.display = 'none'; } 
        else { this.updatePlayerList(); this.els.playerList.style.display = 'flex'; this.focusIndex = -1; this.refreshFocusables(); } 
    },

    updatePlayerList: function() { 
        if(!this.els.playerListContent || typeof Network === 'undefined') return; 
        let myLoc = "[?,?]"; if (Game.state && Game.state.sector) { myLoc = `[${Game.state.sector.x},${Game.state.sector.y}]`; } 
