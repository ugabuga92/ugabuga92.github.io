// [TIMESTAMP] 2026-01-22 15:00:00 - ui_view_town.js - ALERT VERSION
...
Object.assign(UI, {
    ...
    renderCity: function(cityId = 'rusty_springs') {
        ...
        const createCard = (conf) => {
            ...
            // [FIX] Hier rufen wir direkt auf, ohne 'if'. Wenn es fehlt, soll es crashen, damit wir es merken.
    ...
    renderSmithy: function() {
        try {
            ...
            // [FIX] Hier ist der Alert, der dir den Fehler zeigt!
            alert("FATAL ERROR IM SCHMIED:\n" + e.message);
            console.error(e);
        }
    },

    // ... (Hier folgen renderModdingScreen, renderCrafting etc. wie gehabt)
    ...
    // ... (Restlicher Code für Crafting/Shop etc. bleibt identisch, siehe vorherige Version)
    // Damit die Datei valide bleibt, hier die restlichen Funktionen:
        ...
        controlsDiv.innerHTML = `
            <div class="flex flex-col gap-2">
                ...
                <div class="flex gap-2 h-10">
                    ...
                    <button onclick="UI.setShopQty(1)" class="flex-1 border-2 ${getBtnClass(1)} transition-all uppercase tracking-wider">1x</button>
                    <button onclick="UI.setShopQty(10)" class="flex-1 border-2 ${getBtnClass(10)} transition-all uppercase tracking-wider">10x</button>
                    <button onclick="UI.setShopQty('max')" class="flex-1 border-2 ${getBtnClass('max')} transition-all uppercase tracking-wider">MAX</button>
                </div>
            </div>
        ...
        const createRow = (icon, name, qty, price, key, isAmmo = false) => {
            ...
            row.innerHTML = `
                ...
                <div class="h-full flex flex-col justify-center items-end border-l-2 ${colorBorder} bg-black/30 min-w-[100px]">
                    <button class="w-full h-full text-sm font-bold uppercase tracking-wider bg-transparent border-none ${colorBtn}" style="pointer-events: none;">KAUFEN</button>
                </div>
            `;
            return row;
        };
