const WorldGen = {
    ...
    createSector: function(width, height, biomeType, poiList) {
        ...
        // 1. BASIS TERRAIN (Noise Simulation)
        for(let y = 1; y < height - 1; y++) {
            for(let x = 1; x < width - 1; x++) {
                const roll = Math.random();
                if(roll < conf.water) map[y][x] = 'W'; // Wasser
                else if(roll < conf.water + conf.mountain) map[y][x] = 'M'; // Berg
                else if(roll < conf.water + conf.mountain + conf.trees) map[y][x] = 't'; // Baum
                else map[y][x] = conf.ground;
            }
        }

        // 2. POIs PLATZIEREN (Orte müssen erreichbar sein, also Boden planieren)
        poiList.forEach(p => {
            // Bereich um POI säubern
        ...
        // 3. WEGNETZWERK GENERIEREN
        // Wir verbinden alle POIs und die Ränder (Tore) miteinander
        const points = [...poiList];
        
        // Füge ein paar zufällige "Sackgassen"-Ziele hinzu
        for(let i=0; i<3; i++) {
            points.push({x: Math.floor(Math.random()*(width-4))+2, y: Math.floor(Math.random()*(height-4))+2, type: 'nav'});
        }

        // Verbinde Punkte (Drunken Path Algorithm mit Intelligenz)
        for(let i = 0; i < points.length - 1; i++) {
            this.buildRoad(map, points[i], points[i+1], conf.ground);
        }
        // Verbinde letzten mit erstem für Kreiswege
        if(points.length > 1) {
    ...
    buildRoad: function(map, start, end, groundChar) {
        ...
        // Einfacher Pfadfinder
        while(x !== end.x || y !== end.y) {
            // Zufallselement für "natürliche" Wege (nicht immer schnurgerade)
            if(Math.random() < 0.2) {
                const dir = Math.random() < 0.5 ? 0 : 1; // 0: X, 1: Y
            ...
            // INFRASTRUKTUR BAUEN
            if (['V', 'C', 'S', 'H', 'G'].includes(current)) continue; // Nicht POIs überschreiben

            if (current === 'W') {
                map[y][x] = '='; // Brücke über Wasser
            } else if (current === 'M') {
                map[y][x] = 'U'; // Tunnel durch Berg
            } else {
                map[y][x] = '#'; // Straße
            }
            
            // Manchmal breitere Wege oder Lichtung
            if(Math.random() < 0.1) {
                // 3x3 Clearing um Wegpunkt
