// [TIMESTAMP] 2026-01-10 16:15:00 - game_render.js - Pure Map Engine
...
Object.assign(Game, {
    ...
    draw: function() { 
            ...
            for(let x=startX; x<endX; x++) { 
                if(y>=0 && y<this.MAP_H && x>=0 && x<this.MAP_W) { 
                    ...
                    if(!isCity && !this.state.explored[tileKey]) {
                        ctx.fillStyle = "#000"; ctx.fillRect(x * this.TILE, y * this.TILE, this.TILE, this.TILE);
                        continue; 
                    }
                    ...
                    if(this.state.hiddenItems && this.state.hiddenItems[`${x},${y}`]) {
                        ...
                        ctx.globalAlpha = 0.3 + (shimmer * 0.5); ctx.fillStyle = "#ffffff"; ctx.beginPath();
        ...
        if(typeof Network !== 'undefined' && Network.otherPlayers) { 
            for(let pid in Network.otherPlayers) { 
                ...
                ctx.fillStyle = "#00ffff"; ctx.shadowBlur = 5; ctx.shadowColor = "#00ffff"; ctx.beginPath(); ctx.arc(ox, oy, 5, 0, Math.PI*2); ctx.fill(); 
                ctx.font = "10px monospace"; ctx.fillStyle = "white"; ctx.fillText(p.name ? p.name.substring(0,3) : "P", ox+6, oy); ctx.shadowBlur = 0; 
            } 
        } 
        
        const px = this.state.player.x * this.TILE + this.TILE/2; const py = this.state.player.y * this.TILE + this.TILE/2; 
        ctx.save(); ctx.translate(px, py); ctx.rotate(this.state.player.rot); ctx.translate(-px, -py); 
        ctx.fillStyle = "#39ff14"; ctx.shadowBlur = 10; ctx.shadowColor = "#39ff14"; ctx.beginPath(); 
        ctx.moveTo(px, py - 8); ctx.lineTo(px + 6, py + 8); ctx.lineTo(px, py + 5); ctx.lineTo(px - 6, py + 8); ctx.fill(); ctx.shadowBlur = 0; 
    ...
    drawTile: function(ctx, x, y, type, pulse = 1) { 
        ...
        switch(type) { 
            ...
            case 'V': ctx.globalAlpha = 1; ctx.shadowBlur = 10; ctx.shadowColor = "#ffff00"; ctx.fillStyle = "#ffff00"; ctx.textAlign = "center"; ctx.textBaseline = "middle"; ctx.font = "35px monospace"; ctx.fillText("⚙️", px + ts/2, py + ts/2); ctx.font = "bold 10px monospace"; ctx.fillStyle = "#ffffff"; ctx.shadowColor = "#000"; ctx.shadowBlur = 4; ctx.fillText("VAULT 101", px + ts/2, py + ts - 2); ctx.shadowBlur = 0; ctx.textAlign = "start"; ctx.textBaseline = "alphabetic"; break; 
            case 'R': ctx.globalAlpha = 1; ctx.shadowBlur = 8; ctx.shadowColor = "#ff0000"; ctx.fillStyle = "#ff3333"; ctx.textAlign = "center"; ctx.textBaseline = "middle"; ctx.font = "30px monospace"; ctx.fillText("🛒", px + ts/2, py + ts/2); ctx.shadowBlur = 2; ctx.shadowColor = "black"; ctx.font = "bold 9px monospace"; ctx.fillStyle = "#ffffff"; ctx.fillText("SUPER-MART", px + ts/2, py + ts - 2); ctx.shadowBlur = 0; ctx.textAlign = "start"; ctx.textBaseline = "alphabetic"; break;
